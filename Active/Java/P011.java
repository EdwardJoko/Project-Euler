...
public class P011 implements Solution<Integer> {
    public static void main(String[] args) {
        int adjacents = 4;
        System.out.println(new P011().compute(4));
    }
    ...
    public static int max(int[] product) {
        int temp = 0;
        for (int i = 0; i < 4; i++) {
            if (product[i] > temp) temp = product[i];
        }
        return temp;
    }

    public String compute(Integer adjacents) {
        int temp_product = 1;
        ...
        int[] products = {horizontal_product, vertical_product, diagonal_down_right, diagonal_up_right}; 
        int biggestProduct = max(products);
