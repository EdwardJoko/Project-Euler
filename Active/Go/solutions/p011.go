// Project Euler
// Problem 11: The greatest product  of four adjacent numbers in the same
//			   direction (up, down, right, left, and diagonally)
...
func Problem_011(adjacents int) int {
	temp_product := 1

	// horizontal product
	horizontal_product := 1
	for i := 0; i < 20; i++ {
		for j := 0; j <= 20-adjacents; j++ {
			for k := j; k <= j+adjacents-1; k++ {
	...
	temp_product = 1
	for i := 0; i <= 20-adjacents; i++ {
		for j := 0; j <= 20-adjacents; j++ {
			for k := 0; k <= adjacents-1; k++ {
